...
<body>
	<form id="myForm">
		...
		<input type="hidden" name="a_id" value="${addr.a_id}">
		        ...
		        <div class="col-sm-9">
		            <select class="form-control" name="a_province_id" onchange="getCity()">
		            	...
		            	<c:forEach items="${list}" var="l">
		            		<option value="${l.province_id}"
		            			<c:if test="${l.province_id eq addr.a_province_id}">selected</c:if>
		            		>${l.province_name}</option>
		            	</c:forEach>
		            </select>
		        </div>
		    </div>
		    <div class="form-group">
		        ...
		        <div class="col-sm-9">
		            <select class="form-control" name="a_city_id" onchange="getArea()">
		            	<option value="${addr.a_city_id }">${addr.city_name }</option>
		            </select>
		        </div>
		    </div>
		    <div class="form-group">
		        ...
		        <div class="col-sm-9">
		            <select class="form-control" name="a_area_id" onchange="clearError()">
		            	<option value="${addr.a_area_id}">${addr.area_name}</option>
		            </select>
		        </div>
		    </div>
		    ...
		    <div class="form-group">
		        ...
		        <div class="col-sm-9">
		            <input type="text" name="a_desc" class="form-control" value="${addr.a_desc}" onfocus="clearError()"> 
		        </div>
		    </div>
		    <div class="form-group">
		        <label class="col-sm-3 control-label">联系方式</label>
		        <div class="col-sm-9">
		            <input type="number" name="a_tel" class="form-control" maxlength="11" value="${addr.a_tel }" onfocus="clearError()"> 
		        </div>
		    </div>
		    <div class="form-group">
		    	<label class="col-sm-3 control-label"></label>
		    	<div class="col-sm-9">
		    		<button class="btn btn-md btn-primary" style="width:100%;height:35px;font-size:18px" type="button" onclick="toSub()">确 认 修 改</button>
		    	</div>
   ...
   <script>
	   ...
	   function toSub(){
		    ...
		    if(!isPhone(a_tel)){
		   	...
		   	//提交表单
			$("#myForm").attr({"action":"addressEdit.a","method":"post"});
		   	$("#myForm").submit();
