...
<body class="gray-bg">
    		 ...
    		 <div class="col-sm-13" style="margin-top:-15px;margin-bottom:-10px" >
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content" >
                     <div class="row">
                    	<div class="col-md-2"> 
                    		<button class="btn btn-md btn-info" type="button" id="btn"><i class="fa fa-plus"></i> 员工添加</button>
                    	</div>
                    	<div class="col-md-10">
	                    	<form role="form" class="form-inline" method="post" action="userList.do" id="myForm" >
                    ...
                    <a href="javascript:;">
                        <div class="col-sm-4">
                            <div class="text-center">
                                ...
                                <div class="m-t-xs font-bold">${user.role_name}</div>
                               	 <div>
                               	 <button class="btn btn-sm btn-warning" style="margin-top:5px" type="button" onclick="changeRole(${user.user_id},'${user.user_name}',${user.user_role_id})">修改角色</button>
                        ...
                        <div class="col-sm-8">
                           ...
                           <address> 
                            	...
                            	<a href="javascript:;" style="text-decoration:none;cursor:pointer" onclick="changeUserStatus(${user.user_id},'${user.user_name}',${user.user_role_id},${user.user_status})">
	...
	<script>
		...
		function toSub(value){
			...
			window.location.href="userList.do?"+formData+"&pageNow="+$("#pageNow").val();
		}
		...
		//修改用户的角色信息
		  function changeRole(user_id,user_name,user_role_id){
			top.layer.confirm("确定修改【"+user_name+"】的角色状态?",{btn:['确定','取消'],shade:0.3},
			function(){
				if(user_role_id==1){
					top.layer.msg("不得修改超级管理员的角色信息",{icon:5,time:1500});
					top.layer.close();
				}else{
					top.layer.open({
						type:2,
						...
						closeBtn:1,
						area:['420px','220px'],
						content:"userRoleChange.do?user_role_id="+user_role_id+"&user_id="+user_id
					,end:function(){
		...
		//修改用户的状态信息
		function changeUserStatus(user_id,user_name,user_role_id,user_status){
			top.layer.confirm("确定修改【"+user_name+"】的状态信息",{btn:['确定','取消'],shade:0.5},
			function(){
				if(user_role_id==1){
					top.layer.msg("不得修改超级管理员的状态信息",{icon:5,time:1500});
					top.layer.close();
				}else{
					$.post("userChangeStatus.do",{"user_id":user_id,"user_status":user_status},function(data){
						if(data.flag){
							window.location.reload();
							top.layer.msg(data.msg,{icon:1,time:1000});
						}else{
							window.location.reload();
							top.layer.msg(data.msg,{icon:2,time:1000});
						}
					},"json");
				}
			},function(){
