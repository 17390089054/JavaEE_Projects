...
@WebServlet("/userResign.do")
public class UserResign extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		pb = us.queryUserResign(user,pb);
		//查询所有角色信息
		List<Map<String, Object>> roleList = us.queryRole();
		request.setAttribute("roleList",roleList);
		request.setAttribute("pb", pb);
		request.setAttribute("user", user);
		request.getRequestDispatcher("WEB-INF/jsp/sys/user/resigin.jsp").forward(request, response);
	}
